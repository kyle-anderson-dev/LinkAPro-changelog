                        ...
                        <div class="acod-head acc-actives">
                             <h6 class="acod-title text-uppercase">
                                <a data-toggle="collapse" href="#experience-<?php echo esc_html($row->id); ?>" data-parent="#experience-acord" >
                                    ...
                                    <span class="exper-date"><i class="fa fa-clock-o"></i> <?php echo date('M d Y',strtotime($row->start_date)).' -  '.date('M d Y',strtotime($row->end_date)); ?></span>
                                </a>
                             </h6>
                        </div>
				...
				foreach($results as $row){
					...
					?>
						<li>
                            ...
                            <a class="sf-download-certificate" href="<?php echo SERVICE_FINDER_BOOKING_LIB_URL.'/downloads.php?file='.wp_get_attachment_url( $row->attachment_id ) ?>"><i class="fa fa-download"></i> <?php echo esc_html__('View/Download'); ?></a>
                            </div>
                            <?php } ?>
...
<div class="page-content">
  ...
  $classes = (service_finder_themestyle() == 'style-2') ? 'sf-search-result' : '';
  ?>
  <div class="sf-find-bar-inr wow fadeInDown <?php echo $classes; ?> <?php echo sanitize_html_class($positionclass); ?>">
      <div class="container">
      <?php if(service_finder_themestyle() == 'style-2'){ ?>
      <ul class="sf-search-title clearfix">
            <li><?php echo esc_html__('Search Provider', 'service-finder'); ?></li>
      ...
      <div class="row">
        <div class="col-md-4 col-sm-4 col-xs-4">
          <div class="sf-provider-des">
            <div class="sf-thum-bx overlay-black-light sf-feaProgrid-wrap">
              <?php if(service_finder_is_featured($author)){ ?>
              <div class="sf-feaProgrid-label"><?php esc_html_e('Featured', 'service-finder'); ?></div>
              <?php } ?>
			  <?php
  ...
  <?php if(!empty($images)){ ?>
  <section id="sf-provider-gallery" class="section-full overlay-black-middle  text-center gallery-row " style=" <?php echo 'background:url('.$gallerybgurl.') center '.$bgattachment.' no-repeat;'; ?> ">
    <div class="container">
  ...
  <!-- business hours row -->
  <section id="sf-provider-hours" class="section-full text-center bg-white bh-row">
    <div class="container">
  ...
  <?php if($service_finder_options['show-address-info'] && service_finder_check_address_info_access() && service_finder_get_data($service_finder_options,'show-contact-info')){ ?>
  <!-- address-info -->
  <section id="sf-provider-address" class="section-full text-center bg-gray address-info">
    <div class="container">
      <div class="section-head">
        <h2 class="text-uppercase">
        ...
        <?php if(service_finder_get_contact_info_with_text($providerInfo->phone,$providerInfo->mobile) != "" && service_finder_contact_number_is_accessible($author)){ ?>
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              ...
              <p><?php 
			  echo service_finder_get_contact_info_with_text($providerInfo->phone,$providerInfo->mobile); ?></p>
            </div>
          </div>
        <?php } ?>    
        <?php if($providerInfo->fax != "" && $service_finder_options['show-fax']){ ?>    
  ...
  <?php if($service_finder_options['show-contact-map']){ ?> 
  <!-- `-point -->
  <section id="sf-provider-map" class="map-location">
...
<?php
$numberoftabs = array();
if(service_finder_experience_exist($author)){
	$numberoftabs[] = 1;
}
if(service_finder_certificate_exist($author)){
	$numberoftabs[] = 1;
}
if(service_finder_qualification_exist($author)){
	$numberoftabs[] = 1;
}
if(service_finder_amenities_exist($author)){
	$numberoftabs[] = 1;
}
if(service_finder_languages_exist($author)){
	$numberoftabs[] = 1;
}
$totaltabs = count($numberoftabs);
?>
<?php if($totaltabs > 0){ ?>
<?php 
$activeclass = ($totaltabs == 1) ? 'active' : '';
$fadeclass = ($totaltabs == 1) ? 'in' : '';
?>
  <section class="section-full bg-white services-row pro-bg-in">
    <div class="container">
      <div class="tabbable">
        <ul class="nav nav-tabs">
          <?php if(service_finder_experience_exist($author)){ ?>
          <li class="active"><a data-toggle="tab" href="#profilebox1">
            <?php esc_html_e('Experience', 'service-finder'); ?>
            </a></li>
          <?php } ?>  
        ...
        <div class="tab-content">
          <?php if(service_finder_experience_exist($author)){ ?>
          <div id="profilebox1" class="tab-pane fade in active">
            <div class="sf-experience-acord" id="experience-acord">
            <?php
			$results = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$service_finder_Tables->experience. " WHERE `provider_id` = %d ORDER BY ID ASC",$author));
			if(!empty($results)){
			...
			foreach($results as $row){
				...
				echo '<div>';
				esc_html_e('No Experience Available', 'service-finder');
				echo '</div>';
			}	
			?>
            </div>
          ...
          <div id="profilebox2" class="tab-pane fade <?php echo sanitize_html_class($fadeclass).' '.sanitize_html_class($activeclass); ?>">
			...
			<?php
				...
				echo '<div>';
				esc_html_e('No Certificates & Awards Available', 'service-finder');
				echo '</div>';
			}	
            ?>
            </ul>  
          ...
          <div id="profilebox3" class="tab-pane fade <?php echo sanitize_html_class($fadeclass).' '.sanitize_html_class($activeclass); ?>">
			...
			foreach($results as $row){
				...
				echo '<div>';
				esc_html_e('No Amenities and Features Available', 'service-finder');
				echo '</div>';
			}	
			?>
            </div>
          ...
          <div id="profilebox4" class="tab-pane fade <?php echo sanitize_html_class($fadeclass).' '.sanitize_html_class($activeclass); ?>">
            ...
            foreach($amenities as $amenityid){
				...
				echo '<div>';
				esc_html_e('No Qualification Available', 'service-finder');
				echo '</div>';
			}	
            ?>
        	</ul>
          ...
          <div id="profilebox5" class="tab-pane fade <?php echo sanitize_html_class($fadeclass).' '.sanitize_html_class($activeclass); ?>">
            ...
            foreach($languages as $language){
				...
				echo '<div>';
				esc_html_e('No Languages Available', 'service-finder');
				echo '</div>';
			}
            ?>
            </ul>
  ...
  <!-- video-row -->
  <section id="sf-provider-video" class="section-full text-center bg-gray video-row">
    <div class="container">
      <div class="section-head">
        <h2 class="text-uppercase">
          <?php echo (!empty($service_finder_options['label-our-video'])) ? esc_html($service_finder_options['label-our-video']) : esc_html__('Our Video', 'service-finder'); ?>
    ...
    <div class="container">
      <div class="tabbable">
        <ul class="nav nav-tabs">
          ...
          if(!empty($attachmentIDs)){
		  ?>
          <li <?php echo ($serviceflag == 0) ? 'class="active"' : ''; ?>><a data-toggle="tab" href="#box2">
            <?php echo (!empty($service_finder_options['label-documents'])) ? esc_html($service_finder_options['label-documents']) : esc_html__('Documents', 'service-finder'); ?>
            </a></li>
          <?php } ?>  
        </ul>
        <div class="tab-content">
          <?php if($service_finder_options['my-services-menu']){ ?>
          ...
          <div id="box2" class="tab-pane fade <?php echo ($serviceflag == 0) ? 'in active' : ''; ?>">
            <?php 
    ...
    <div class="container">
      <div class="tabbable">
        ...
        <div class="tab-content">
          ...
          <?php } ?>
          <?php if(service_finder_article_exist($author)){ ?>
          <div id="qa4" class="tab-pane fade <?php echo (!service_finder_get_data($service_finder_options,'review-system')  && !service_finder_get_data($service_finder_options,'question-answer-section')) ? 'in active' : ''; ?>">
            <?php require SERVICE_FINDER_BOOKING_TEMPLATES_DIR . '/articles.php'; ?>
          </div>
          <?php } ?>
        </div>
