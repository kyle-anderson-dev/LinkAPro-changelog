...
if(!class_exists('service_finder_booking_plugin')) {
	class service_finder_booking_plugin {
		...
		public function __construct() {
			...
			add_action('service_finder_check_purchasecredit_expirations', array(&$this, 'service_finder_CheckPurchaseCreditExpirations'));
		...
		public static function service_finder_activate() {
			...
			if ( !wp_next_scheduled( 'service_finder_check_purchasecredit_expirations' ) ){
				wp_schedule_event( time(), 'daily', 'service_finder_check_purchasecredit_expirations' );	
			}
		...
		public static function service_finder_deactivate() {
			...
			wp_clear_scheduled_hook('service_finder_check_purchasecredit_expirations');
		...
		**********************/
		public function service_finder_init(){
			...
			/*if($ratingstyle == 'custom-rating'){
			add_action( 'booking_rating_form', 'service_finder_booking_rating_form', 1 );
			}else{
			add_action( 'booking_rating_form', 'service_finder_booking_rating_form', 1 );		
			}*/
			...
			$service_finder_Tables = (object) array(
										...
										'purchase_credit' => 'service_finder_purchase_credit',
			...
			/*Purchase credit limits*/
			$wpdb->query(
				"CREATE TABLE IF NOT EXISTS `".$service_finder_Tables->purchase_credit."` (
					`id`                 INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
					`provider_id` 		 INT UNSIGNED NOT NULL,
					`free_limits`	 	 INT UNSIGNED NOT NULL,
					`paid_limits` 		 INT UNSIGNED NOT NULL,
		...
		**********************/
		...
		public function service_finder_CheckPurchaseCreditExpirations() {
			global $service_finder_options, $wpdb, $service_finder_Tables;
			
				$joblimits = $wpdb->get_results("SELECT * FROM ".$service_finder_Tables->purchase_credit);
				foreach ($joblimits as $joblimit) {
					...
					if($differenceInSec >= 0){
						...
						$appendlimit = (!empty($service_finder_options['append-purchase-credit'])) ? $service_finder_options['append-purchase-credit'] : '';
				...
				if(!empty($results)){
					...
					if(function_exists('service_finder_add_notices')) {
						...
						$noticedata = array(
								...
								'target_id' => $row->id, 
								'topic' => esc_html__('Payout status', 'service-finder'),
...
var dowlabels = ["'.esc_html__('Mon','service-finder').'", "'.esc_html__('Tue','service-finder').'", "'.esc_html__('Wed','service-finder').'", "'.esc_html__('Thu','service-finder').'", "'.esc_html__('Fri','service-finder').'", "'.esc_html__('Sat','service-finder').'", "'.esc_html__('Sun','service-finder').'"];', 'before' );
			...
			if(is_author() || is_search() || is_page()){
			/*Search form js*/
			...
			wp_register_style('sf-jquery-ui', 'https://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css','',null);
			wp_register_script('sf-jquery-ui', 'https://code.jquery.com/ui/1.12.1/jquery-ui.js', array('jquery') , null, true);
			wp_register_script('sf-job-search', plugins_url('/sf-booking/') . '/frontend/modules/search/resources/job-search-form.js', array('jquery') , null, true);
		...
		**********************/
		...
		public function service_finder_admin_scripts() {
			...
			wp_enqueue_script('bootstrap-select', plugins_url('/sf-booking/') . '/js/bootstrap-select.min.js', array('jquery') , null, false);
			}
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$reset_link = '<a href="' . esc_url($reset_url) . '" style="display:table; padding:6px 40px; border:2px solid #555; margin:0px auto 30px; color:#555; font-size:16px;">CLICK HERE</a>';
	
	$replacements = array($user->user_login,$reset_link);		
	
	$msg_body = str_replace($tokens,$replacements,$message);
	
	service_finder_wpmailer($user->user_email,$subject,$msg_body,'yes');
}
}
