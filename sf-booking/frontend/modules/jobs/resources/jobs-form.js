...
// When the browser is ready...
  ...
  jQuery(function() {
	...
	jQuery("#myTab a").click(function(e){
		...
		load_job_limit_datatable(tabid);
	});

	function load_job_limit_datatable(tabid){
		if(tabid == '#job-limits'){
	...
	.on('success.form.bv', function(form) {
			...
			if(woopayment && woooption != "wallet" && woooption != "skippayment"){
	...
	jQuery("#myTab a").click(function(e){
		...
		jQuery(this).tab('show');
		var tabid = jQuery(this).attr('href');
		load_jobs_datatable(tabid);
	});
	
	function load_jobs_datatable(tabid){
		if(tabid == '#my-jobs'){
