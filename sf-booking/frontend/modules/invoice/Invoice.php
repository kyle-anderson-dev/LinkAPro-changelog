...
$availablepaymentmethod .= service_finder_add_wallet_option('invoicepayment_mode','invoice');
		...
		$paybtn = '<input name="invoicepayment" id="invoicepayment" type="submit" value="Pay Now" class="btn btn-primary center-block">';
		}else{
			...
			if ( ! $res) {
				...
				if(function_exists('service_finder_add_notices')) {
					...
					$noticedata = array(
							...
							'notice' => sprintf( esc_html__('Previousaly generated invoice have been updated. Please review it. Invoice Ref id is #%d', 'service-finder'), ucfirst(service_finder_getProviderName($user_id)), $arg['refno'] )
							);
					service_finder_add_notices($noticedata);
