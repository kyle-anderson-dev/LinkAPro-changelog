	...
	public function service_finder_view_quotation_details($arg){
		...
		$attachments = (!empty($row->attachments)) ? unserialize($row->attachments) : '';
			...
			if(!empty($attachments)){
				...
				$attachfile .= '<a href="'.SERVICE_FINDER_BOOKING_LIB_URL.'/downloads.php?file='.wp_get_attachment_url( $attachmentid ).'"><i class="fa fa-download"></i> '.esc_html__('View/Download').'</a><br/>';
				}
			}
